...
const translations = {
  EN: {
    management: "Attendances Management",
    title: "Update",
    subtitle: "Update selected attendance information.",
  ...
  FR: {
    ...
    title: "Modifier",
    subtitle: "Mettre à jour les informations de présence sélectionnées.",
  ...
  AR: {
    ...
    title: "تعديل",
    subtitle: "تحديث معلومات الحضور المحددة.",
...
export default function EditAttendance({
  ...
  const studentName =
    ...
    formData.fullName ||
    `${formData.studentPrenom || ""} ${formData.studentNom || ""}`.trim() ||
    `${formData.student?.prenom || ""} ${formData.student?.nom || ""}`.trim() ||
      ...
      >
        ...
        <form onSubmit={onSubmit} className="grid gap-5 rounded-b-[2rem] p-6">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
            <ReadOnlyField label={t.studentName} value={studentName} />
            ...
            <StatusField t={t} value={formData.status || ""} onChange={onChange} />
  ...
  return (
    <div>
      ...
      <input
        ...
        readOnly
        disabled
        className="w-full cursor-not-allowed select-none rounded-2xl border px-4 py-2.5 text-sm font-bold outline-none"
        style={{
...
function StatusField({ t, value, onChange }) {
  return (
    <div>
      <label
        className="mb-2 block text-xs font-black"
...
function ModernDatePicker({ label, value, onChange, t, isArabic }) {
  ...
  const monthName = useMemo(() => {
    return viewDate.toLocaleDateString(isArabic ? "ar-MA" : "fr-FR", {
      month: "long",
      year: "numeric",
    });
  }, [viewDate, isArabic]);
